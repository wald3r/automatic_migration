...
def generate_test_data(df, scaler):
    ...
    df_tmp = df[['mean', 'min', 'max', 'count', 'mad', 'median', 'sum']]
    df_tmp = df_tmp.tail(30)


    df_total = df[['mean', 'min', 'max', 'count', 'mad', 'median', 'sum']]
...
def main():
    ...
    predictions100, model = train_and_predict(model, training_features, test_features, labels, epochs100-epochs10, scaler)
    predictions250, model = train_and_predict(model, training_features, test_features, labels, epochs250-epochs100, scaler)
    #predictions300, model = train_and_predict(model, training_features, test_features, labels, epochs300-epochs250, scaler)
    #predictions500, model = train_and_predict(model, training_features, test_features, labels, epochs500-epochs300, scaler)
    ...
    #predictions, model = train_and_predict(model, training_features, test_features, labels, epochs, scaler)


    print_errors(predictions10[:,6], test_data[:,6], epochs10)
    print_errors(predictions100[:,6], test_data[:,6], epochs100)
    print_errors(predictions250[:,6], test_data[:,6], epochs250)
    #print_errors(predictions300[:,11], test_data[:,11], epochs300)
    #print_errors(predictions500[:,11], test_data[:,11], epochs500)
    #print_errors(predictions750[:,6], test_data[:,6], epochs750)
    #print_errors(predictions1000, test_data[:,6], epochs1000)

    #save_model(model)

    plot_all(predictions250[:,6], test_data[:,6], epochs250)
