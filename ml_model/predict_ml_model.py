...
def generate_training_data(df, availability_zone):
    ...
    df_tmp = df_tmp.head(len(df_tmp)-test)
    ...
    for i in range(ticks, len(scaled_data)):
        features_set.append(scaled_data[i - ticks:i])
...
def generate_test_data(df, scaler, availability_zone):
    ...
    test_data = df_total[len(df_total) - len(df_tmp) - ticks:].values
    test_data = scaler.transform(test_data)

    test_features = []
    for i in range(ticks, len(test_data)):
        test_features.append(test_data[i - ticks:i])
